...
- For a Zerodha-style broker tax-P&L, aggregate the heads deterministically with
  ...
  Exits" sheet to CSV (this is a **source-derived input** — keep it in `sources/` with the other source
  documents, not in the pipeline's output folder), then `schedule_cg.ps1 -Path <csv> -OutDir out` writes per-head
